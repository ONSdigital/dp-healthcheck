...
func TestHealthcheckFilter(t *testing.T) {
	...
	Convey("Given a HealthcheckFilter generated from a healthcheck object's handler", t, func() {
		...
		allowedMap := HealthcheckFilter(hc.Handler)

		Convey("Then the expected Allowed structure is created for /health endpoint", func() {
			So(len(allowedMap), ShouldEqual, 1)
			So(allowedMap["/health"].Methods, ShouldResemble, []string{http.MethodGet})
			So(allowedMap["/health"].Handler, ShouldEqual, hc.Handler)
		})
	})
}
...
func TestWhitelist(t *testing.T) {
	...
	Convey("Given a generic Allowed structure with methods and a whitelisted Handler function", t, func() {
